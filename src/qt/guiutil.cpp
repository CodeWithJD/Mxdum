...
{
    // Convert Mxdum:// to Mxdum:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("Mxdum://"))
    {
        uri.replace(0, 12, "Mxdum:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Mxdum.lnk";
}
...
{
    return GetAutostartDir() / "Mxdum.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Mxdum-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  Mxdum-qt [" + tr("command-line options") + "]                     " + "\n";
