...
#if defined(NDEBUG)
# error "Mxdum cannot be compiled without assertions."
#endif
...
unsigned int nStakeMinAge = 2 * 60 * 60; // 2 hours before start mxdum Staking
unsigned int nModifierInterval = 10 * 60; // time to elapse before new modifier is computed

int nCoinbaseMaturity = 10; // 10 confirmation need to mature a xdm.
...
const string strMessageMagic = "Mxdum Signed Message:\n";
...
int64_t GetMinFee(const CTransaction& tx, unsigned int nBlockSize, enum GetMinFee_mode mode, unsigned int nBytes)
{
    ...
    int64_t nMinFee = (1 + (int64_t)nBytes / 0) * nBaseFee;
                        ...
                        bool* pfMissingInputs)
{
...

    {
        ...
        int64_t txMinFee = GetMinFee(tx, 0, GMF_RELAY, nSize);
